...
import codecs, locale, os, re, subprocess
...
class Test():
    ...
    def run_test(self, compiler='', fail_tests_for_errors=True):
        ...
        if stdin:
            stdin.close()
        self.stdout = codecs.decode(stdout, locale.getpreferredencoding(False), errors='replace')
        self.stderr = codecs.decode(stderr, locale.getpreferredencoding(False), errors='replace')

        self.short_explanation = None
        self.long_explanation = None

        if fail_tests_for_errors:
    ...
    def check_files(self):
        for expected_file in self.parameters.get('expected_files', []):
            ...
            try:
                ...
                self.long_explanation = "Your program was expected to create a file named %s and did not" % filename
...
#       colored = lambda x,*a,**kw: x # disable use of blue below - hard to read, replace with more readable color
        ...
        if show_reproduce_command and not self.parameters.get('no_reproduce_command', ''):
            ...
            if input:
                if not self.stdin_file_name() or (len(input) < 64 and n_input_lines < 4 and '"' not in input):
                    input = input.rstrip('\n')
                    echo = "echo -e" if '\n' in input else "echo"
                    input = input.replace('\n','\\n').replace('"','\\"')  # shlex.quote ??
                    command = '%s "%s" | %s' % (echo, input, command)
                else:
