...
def fetch_tests_from_autotest_directory(autotest_dir, debug=0):
    tests = collections.OrderedDict()
    global_variables = {'debug':debug}
    try:
        filename = os.path.join(autotest_dir, 'tests.txt')
...
if __name__ == '__main__':
    ...
    tests = fetch_tests_from_autotest_directory('.')
    ...
    try:
        for test in tests.values():
            ...
            if not os.path.exists(test.program): # handle globs
                    ...
                    if not os.path.exists(file):
                        ...
                        for name in [file, alternate_name]:
                            ...
                            if p.stdout:
                                ...
                                print('cp', path, file, file=sys.stderr)
                                shutil.copyfile(path, file)
                                shutil.copymode(path, file)
                                unlink_files.append(file)
                                break
