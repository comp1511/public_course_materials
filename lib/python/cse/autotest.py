...
import  argparse, atexit, copy, glob, io, json, re, os, paramiko, requests, shutil, signal, subprocess, sys, tempfile, traceback, zipfile
from collections import OrderedDict

from cse.course_configuration import course_configuration
from cse.parse_autotest_directory import fetch_tests_from_autotest_directory,interpolate_backquotes,make_list
...
def main():
    ...
    if not args.autotest_directory and 'DRYRUN_DIR' in os.environ:
        ...
        if args.marking:
             args.autotest_directory = find_autotest_dir(args.exercise, autotest_subdir=['automarking', 'tests'])
        else:
    ...
    tests = fetch_tests_from_autotest_directory(args.autotest_directory, debug=debug)
    if not tests:
    ...
    copy_tree(args.autotest_directory, temp_dir, preserve_symlinks=0)
...
def run_tests(tests, args, file=sys.stdout):
    ...
    for test_label in tests:
        if test_label in args.labels:
            ...
            missing_files = [f for f in test.files if not os.path.exists(f)]
            if missing_files:
            ...
            if compile_commands[test.program]:
                ...
                for (i, pre_execute_command) in enumerate(pre_execute_commands[test.program]):
                    ...
                    if 'setup_command' in test.parameters:
                        ...
                        run(test.parameters['setup_command'], shell=test.parameters.get('setup_command', False))
                    pre_execute_command()
...
def find_autotest_dir(submission_name, autotest_subdir=['autotest','dryrun']):
    ...
    for dir in ['.', d, os.path.join(d, '*'), os.path.join(d, '*', '*')]:
        for name in names:
